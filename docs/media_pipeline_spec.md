...
- `AssetFetcher`（`src/assets/pipeline.py`）が検索・ダウンロード・キャッシュ保存を一括管理。`scripts/fetch_assets.py` で CLI から呼び出せる。
- 実運用では `scripts/generate_video.py` の `ensure_background_assets()` が不足素材の補完を自動実行（キーワードのサニタイズと 400/429 回避ロジックを含む）。

## 2. 素材整形
- 画像は ffmpeg の `zoompan` や Ken Burns 風プリセットで動きを付ける。
- 動画は `-stream_loop` で尺を揃え、音声はミュート。
- ロゴ／アイコン／ランキング番号などのオーバーレイ用素材を準備し、テロップと一緒に描画（`src/models.Section.overlays`）。
...
## 4. ワークフロー
1. テーマ入力→キーワード生成（必要なら LLM を利用）。
2. 素材検索→キャッシュ保存（不足時は `ensure_background_assets` が自動補完）。
3. ffmpeg で背景／テロップ合成。
4. Pillow でサムネを生成。
