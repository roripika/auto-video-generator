...
- **動画生成時にデフォルトで自動実行**（`--adjust-tickers` がデフォルト有効）。無効化したい場合は `--no-adjust-tickers` を指定。
...
- 2025-12-23: **重要修正** - `generate_video.py` で `--adjust-tickers` をデフォルト有効化。動画生成時に自動的にテロップ調整が実行されるように変更。無効化する場合は `--no-adjust-tickers` を指定。
